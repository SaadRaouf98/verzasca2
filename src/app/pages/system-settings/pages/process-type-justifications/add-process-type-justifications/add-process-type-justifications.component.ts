...
@Component({
  selector: 'app-add-process-type-justifications',
  templateUrl: './add-process-type-justifications.component.html',
  styleUrls: ['./add-process-type-justifications.component.scss'],
})
export class AddProcessTypeJustificationsComponent {
  form!: FormGroup;
  elementId: string = '';
  processTypeJustificationElement$: Observable<Entity> = new Observable<Entity>();
  lang: string = 'ar';
  ...
  ngOnInit(): void {
    ...
    if (this.elementId) {
      this.processTypeJustificationElement$ =
        this.manageActionsService.processTypeJustificationsService.getProcessTypeJustificationById(
          this.elementId
        );
    }
    this.setFormMode();
  ...
  onSubmit(data: any): void {
    if (this.elementId) {
      this.manageActionsService.processTypeJustificationsService
        .updateProcessTypeJustification(data)
        .subscribe({
          next: (res) => {
      ...
      delete data.id;
      this.manageActionsService.processTypeJustificationsService
        .addProcessTypeJustification(data)
        .subscribe({
          next: (res) => {
