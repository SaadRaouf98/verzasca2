...
@Component({
  selector: 'app-process-type-justifications-list',
  templateUrl: './process-type-justifications-list.component.html',
  styleUrls: ['./process-type-justifications-list.component.scss'],
})
export class ProcessTypeJustificationsListComponent {
  processTypeJustificationsSource: MatTableDataSource<Entity> = new MatTableDataSource<Entity>([]);
  pageIndex: number = 0;
  pageSize: number = 20;
  ...
  initializeTable(): Observable<AllEntities> {
    this.isLoading = true;
    return this.manageActionsService.processTypeJustificationsService
      ...
      .pipe(
        tap((res) => {
          ...
          this.processTypeJustificationsSource = new MatTableDataSource(res.data);
    ...
    this.dialog.open(ConfirmationModalComponent, {
      ...
      data: {
        ...
        content: `${this.translateService.instant(
          'SystemSettingsModule.ProcessTypeJustificationsListComponent.deleteJustificationWarning'
        ...
        confirmationAction: () => {
          ...
          this.manageActionsService.processTypeJustificationsService
            .deleteProcessTypeJustification(element.id)
            .subscribe((res) => {
  ...
  setPermissionProperties(): void {
    if (this.authService.userPermissions.includes(PermissionsObj.UpdateProcessTypeJustification)) {
      this.canEdit = true;
    }

    if (this.authService.userPermissions.includes(PermissionsObj.DeleteProcessTypeJustification)) {
      this.canDelete = true;
    }
  }
}
