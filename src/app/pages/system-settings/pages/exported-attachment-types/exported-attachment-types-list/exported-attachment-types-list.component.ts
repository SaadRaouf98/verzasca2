...
@Component({
  selector: 'app-exported-attachment-types-list',
  templateUrl: './exported-attachment-types-list.component.html',
  styleUrls: ['./exported-attachment-types-list.component.scss'],
  animations: [
...
export class ExportedAttachmentTypesListComponent {
  exportedAttachmentTypesSource: MatTableDataSource<ExportedAttachmentType> =
    new MatTableDataSource<ExportedAttachmentType>([]);
  pageIndex: number = 0;
  pageSize: number = 20;
  ...
  expandedElement!: ExportedAttachmentType | null;
  ...
  initalizeTable(): Observable<AllExportedAttachmentTypes> {
    this.isLoading = true;
    return this.manageActionsService.exportedAttachmentTypesService
      ...
      .pipe(
        tap((res) => {
          ...
          this.exportedAttachmentTypesSource = new MatTableDataSource(res.data);
    ...
    this.dialog.open(ConfirmationModalComponent, {
      ...
      data: {
        headerTranslationRef: this.translateService.instant(
          'SystemSettingsModule.ExportedAttachmentTypesListComponent.confirmDeletion'
        ),
        headerIconSrc: 'assets/icons/trash-solid.svg',
        hasActionButtons: true,
        hasDeleteBtn: true,
        content: `${this.translateService.instant(
          'SystemSettingsModule.ExportedAttachmentTypesListComponent.deleteAttachmentTypeWarning'
        ...
        confirmationAction: () => {
          ...
          this.manageActionsService.exportedAttachmentTypesService
            .deleteExportedAttachmentType(element.id)
            .subscribe({
              next: (res) => {
  ...
  view_hide_element(element: ExportedAttachmentType): void {
  ...
  check_view_element(element: ExportedAttachmentType): boolean {
    if (JSON.stringify(this.expandedElement) === JSON.stringify(element)) {
      return true;
    } else {
      return false;
  ...
  return_displayed_columns(): string[] {
    if (isSmallDeviceWidthForTable()) {
      ...
      return ['title', 'titleEn', 'description', 'descriptionEn', 'isRequiredNumber', 'actions'];
    }
  }
}
