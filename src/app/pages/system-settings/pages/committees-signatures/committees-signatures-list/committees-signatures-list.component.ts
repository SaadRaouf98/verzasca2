...
@Component({
  selector: 'app-committees-signatures-list',
  templateUrl: './committees-signatures-list.component.html',
  styleUrls: ['./committees-signatures-list.component.scss'],
  animations: [
...
export class CommitteesSignaturesListComponent {
  signaturesSource: MatTableDataSource<Signature> = new MatTableDataSource<Signature>([]);
  pageIndex: number = 0;
  pageSize: number = 20;
  ...
  initializeTable() {
    this.isLoading = true;
    return this.manageActionsService.signaturesService
      .getSignaturesList(
      ...
      .pipe(
        tap((res) => {
          ...
          this.signaturesSource = new MatTableDataSource(res.data);
  ...
  onDeleteElement(element: Signature): void {
    this.dialog.open(ConfirmationModalComponent, {
      ...
      data: {
        ...
        confirmationAction: () => {
          ...
          this.manageActionsService.signaturesService.deleteSignature(element.id).subscribe({
            next: (res) => {
  ...
  view_hide_element(element: Signature): void {
  ...
  check_view_element(element: Signature): boolean {
    if (JSON.stringify(this.expandedElement) === JSON.stringify(element)) {
      return true;
    } else {
      return false;
  ...
  return_displayed_columns(): string[] {
    if (isSmallDeviceWidthForTable()) {
      ...
      return ['title', 'committee', 'membersCount', 'status', 'actions'];
    }
  }
}
