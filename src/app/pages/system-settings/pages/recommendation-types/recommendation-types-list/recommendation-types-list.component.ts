...
@Component({
  selector: 'app-recommendation-types-list',
  templateUrl: './recommendation-types-list.component.html',
  styleUrls: ['./recommendation-types-list.component.scss'],
})
export class RecommendationTypesListComponent {
  recommendationTypesSource: MatTableDataSource<Entity> = new MatTableDataSource<Entity>([]);
  pageIndex: number = 0;
  pageSize: number = 20;
  ...
  initalizeTable(): Observable<AllEntities> {
    this.isLoading = true;
    return this.manageActionsService.recommendationTypesService
      ...
      .pipe(
        tap((res) => {
          ...
          this.recommendationTypesSource = new MatTableDataSource(res.data);
    ...
    this.dialog.open(ConfirmationModalComponent, {
      ...
      data: {
        headerTranslationRef: this.translateService.instant(
          'ActionsModule.RecommendationTypesListComponent.confirmDeletion'
        ),
        headerIconSrc: 'assets/icons/trash-solid.svg',
        hasActionButtons: true,
        hasDeleteBtn: true,
        content: `${this.translateService.instant(
          'ActionsModule.RecommendationTypesListComponent.deleteRecommendationTypeWarning'
        ...
        confirmationAction: () => {
          ...
          this.manageActionsService.recommendationTypesService
            .deleteRecommendationType(element.id)
            .subscribe((res) => {
  ...
  setPermissionProperties(): void {
    if (this.authService.userPermissions.includes(PermissionsObj.UpdateRecommendationType)) {
      this.canEdit = true;
    }

    if (this.authService.userPermissions.includes(PermissionsObj.DeleteRecommendationType)) {
      this.canDelete = true;
    }
  }
}
