...
export class ReferralJustificationsService {
  readonly apiUrl = '/api/v1/referraljustifications';
  ...
  getReferralJustificationById(id: string): Observable<Entity> {
    let url = `${this.apiUrl}/${id}`;
    return this.apiService.get(url);
  }

  addReferralJustification(data: Entity): Observable<any> {
    return this.apiService.post(`${this.apiUrl}`, data);
  }

  updateReferralJustification(data: Entity): Observable<any> {
    return this.apiService.put(`${this.apiUrl}`, data);
  }

  deleteReferralJustification(id: string): Observable<any> {
