...
export class BenefitTypesService {
  readonly apiUrl = '/api/v1/benefittypes';
  ...
  getBenefitTypeById(id: string): Observable<Entity> {
    let url = `${this.apiUrl}/${id}`;
    return this.apiService.get(url);
  }

  addBenefitType(data: Entity): Observable<any> {
    return this.apiService.post(`${this.apiUrl}`, data);
  }

  updateBenefitType(data: Entity): Observable<any> {
    return this.apiService.put(`${this.apiUrl}`, data);
  }

  deleteBenefitType(id: string): Observable<any> {
