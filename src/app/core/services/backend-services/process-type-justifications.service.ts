...
export class ProcessTypeJustificationsService {
  readonly apiUrl = '/api/v1/processtypejustifications';
  ...
  getProcessTypeJustificationById(id: string): Observable<Entity> {
    let url = `${this.apiUrl}/${id}`;
    return this.apiService.get(url);
  }

  addProcessTypeJustification(data: Entity): Observable<any> {
    return this.apiService.post(`${this.apiUrl}`, data);
  }

  updateProcessTypeJustification(data: Entity): Observable<any> {
    return this.apiService.put(`${this.apiUrl}`, data);
  }

  deleteProcessTypeJustification(id: string): Observable<any> {
