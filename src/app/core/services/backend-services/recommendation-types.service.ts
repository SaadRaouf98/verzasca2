...
export class RecommendationTypesService {
  readonly apiUrl = '/api/v1/recommendationtypes';
  ...
  getRecommendationTypeById(id: string): Observable<Entity> {
    let url = `${this.apiUrl}/${id}`;
    return this.apiService.get(url);
  }

  addRecommendationType(data: Entity): Observable<any> {
    return this.apiService.post(`${this.apiUrl}`, data);
  }

  updateRecommendationType(data: Entity): Observable<any> {
    return this.apiService.put(`${this.apiUrl}`, data);
  }

  deleteRecommendationType(id: string): Observable<any> {
